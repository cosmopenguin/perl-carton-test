# This Makefile is for the PerlCartonTest::File extension to perl.
#
...
DIRFILESEP = /
DFSEP = $(DIRFILESEP)
NAME = PerlCartonTest::File
NAME_SYM = PerlCartonTest_File
VERSION = 0
...
MAKE = make
FULLEXT = PerlCartonTest/File
BASEEXT = File
PARENT_NAME = PerlCartonTest
DLBASE = $(BASEEXT)
...
INST_LIBDIR      = $(INST_LIB)/PerlCartonTest
INST_ARCHLIBDIR  = $(INST_ARCHLIB)/PerlCartonTest
...
DISTNAME = PerlCartonTest-File
DISTVNAME = PerlCartonTest-File-0
...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name: PerlCartonTest-File' >> META_new.yml
	...
	$(NOECHO) $(ECHO) '   },' >> META_new.json
	$(NOECHO) $(ECHO) '   "name" : "PerlCartonTest-File",' >> META_new.json
...
ppd :
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="PerlCartonTest-File" VERSION="0">' > PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT></ABSTRACT>' >> PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '    <AUTHOR></AUTHOR>' >> PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '    <IMPLEMENTATION>' >> PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '        <ARCHITECTURE NAME="x86_64-linux-gnu-thread-multi-5.30" />' >> PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '        <CODEBASE HREF="" />' >> PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '    </IMPLEMENTATION>' >> PerlCartonTest-File.ppd
	$(NOECHO) $(ECHO) '</SOFTPKG>' >> PerlCartonTest-File.ppd
...
pm_to_blib : $(FIRST_MAKEFILE) $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', q[$(PM_FILTER)], '\''$(PERM_DIR)'\'')' -- \
	  'File.pm' '$(INST_LIB)/PerlCartonTest/File.pm' 
